Feature:Saves the access token to OAuth2 profile when valid OAuth 2.0 parameters (authorization URI, access token URI,
  ...
  Scenario: Access token is saved for google tasks api
  ...
  Scenario: Access token is overwritten if exists already
    Given Uwe Krull created a REST project with <RestURI>
    And an OAuth2 profile is created with <authURI>, <accessTokenURI>, <redirectUri>, <scope>, <clientID>, <clientSecret>
  ...
  Scenario: Error message is displayed if the OAuth parameters are not valid.
    Given Uwe Krull created a REST project with <RestURI>
    And an OAuth2 profile is created with <authURI>, <accessTokenURI>, <redirectUri>, <scope>, <clientID>, <clientSecret>
    When user clicks on get access token
  ...
  Examples:
  | RestURI                                             | authURI                                    | accessTokenURI                             | redirectUri                  | scope                                            | clientID                                   | clientSecret             |
  | ftp://www.googleapis.com/tasks/v1/users/@me/lists   | https://accounts.google.com/o/oauth2/auth  | https://accounts.google.com/o/oauth2/token | http://localhost:8080/       | https://www.googleapis.com/auth/tasks.readonly   | 669184148999.apps.googleusercontent.com    | vqWu7TpONtgABF2Ooay4fODG |
  | https://www.googleapis.com/tasks/v1/users/@me/lists | htttps://accounts.google.com/o/oauth2/auth | https://accounts.google.com/o/oauth2/token | urn:ietf:wg:oauth:2.0:oob    | https://www.googleapis.com/auth/tasks.readonly   | 669184148999.apps.googleusercontent.com    | vqWu7TpONtgABF2Ooay4fODG |

  Scenario: Error message is displayed when access token was not fetched due to some problems.
    Given Uwe Krull created a REST project with <RestURI>
    And a OAuth2 profile is created with <authURI>, <accessTokenURI>, <scope>, <clientID>, <clientSecret> <accessToken>
    When user clicks on get access token
    Then access token is not fetched and an error message is displayed on the GUI.