                    ...
                    if (mediaType.getExamples() != null) {
                        for (String exampleName : mediaType.getExamples().keySet()) {
                            ...
                            if (addRequestContent) {
                                ...
                                if (StringUtils.isNullOrEmpty(representation.getSampleContent())) {
            ...
            for (String httpStatusCode : operation.getResponses().keySet()) {
                ...
                if (response.getContent() != null) {
                    for (Map.Entry<String, MediaType> contentItem : response.getContent().entrySet()) {
                        ...
                        MediaType mediaType = contentItem.getValue();
                        if (mediaType.getExample() != null) {
                            representation.setSampleContent(OpenAPIUtils.extractStringFromExampleObject(mediaTypeName, mediaType.getExample()));
                        } else {
                            Schema mediaTypeSchema = mediaType.getSchema();
                            if (mediaTypeSchema != null) {
                                Map<String, Schema> schemas = getOpenApiSchemas();
                                io.swagger.oas.inflector.examples.models.Example example = ExampleBuilder.fromSchema(mediaTypeSchema, schemas, ExampleBuilder.RequestType.READ);
                                if (example != null) {
                                    representation.setSampleContent(serializeExample(mediaTypeName, example));
                                }
                            }
                        }
                    }
