...
public class RestRequest extends AbstractHttpRequest<RestRequestConfig> implements RestRequestInterface
{
	...
	private void setOriginalUriInConfig()
	{
		...
		{
			if( getResponse() != null && getResponse().getURL() != null )
			{
				getConfig().setOriginalUri( getResponse().getURL().toString() );
			}
			else if (StringUtils.hasContent( getEndpoint() ) && StringUtils.hasContent( getPath() ))
			{
				// remove the port to match the namespace URIs generated by JsonMediaTypeHandler
				String endpoint = removePortFrom( getEndpoint() );
				getConfig().setOriginalUri( endpoint + getPath() );
			}
		}
	}

	private String removePortFrom( String endpoint )
	{
		return endpoint.replaceAll(":\\d{1,5}$", "");
	}
