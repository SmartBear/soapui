...
{
	public static String checkForExplicitReference( String propertyName, String prefix, 
				TestPropertyHolder holder, PropertyExpansionContext context, boolean globalOverride )
	{
		if( holder == null )
			return null; 
	...
	public static String parseProperty( String name, TestPropertyHolder holder, PropertyExpansionContext context, boolean globalOverride )
	{
		...
		if( sepIx != -1 )
		{
			...
			{
				return context == null ? 
					ResolverUtils.extractXPathPropertyValue( property, xpath ) :
					ResolverUtils.extractXPathPropertyValue( property, PropertyExpansionUtils.expandProperties( context, xpath ) );
			}
		}
		else
		{
	...
	public static String extractXPathPropertyValue( Object property, String xpath )
	{
		try
		{
			...
			if( paths.length > 0 )
			{
				...
				if( domNode.getNodeType() == Node.ELEMENT_NODE && XmlUtils.getChildElements((Element) domNode).getLength() > 0 )
					return paths[0].xmlText(new XmlOptions().setSaveOuter());
				else
