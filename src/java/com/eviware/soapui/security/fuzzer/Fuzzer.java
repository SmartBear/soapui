...
/**
 * A simple Fuzzer implementation. This will also provide different fuzzer
 * instances IF we find that this class gets to complex, we could refactor it to
 * use Fuzzer registry
 * 
 * @author nenad.ristic
 * 
...
public class Fuzzer {
	...
	/**
	 * Gets the TestStep with the appropriate parameter fuzzed with the next
	 * fuzz value Later we can make this more complex to use various encodings
	 * 
	 * @param testStep
	 *            The TestStep to fuzz
	 * @param param
		...
		if (currentIndex < config.getValueList().size()) {
			if (testStep instanceof WsdlTestRequestStep) {
				AbstractHttpRequest<?> request = ((WsdlTestRequestStep) testStep)
						.getHttpRequest();
				String newContent = XmlUtils.setXPathContent(request
				...
				HttpTestRequestInterface<?> request = ((HttpTestRequestStepInterface) testStep)
						.getTestRequest();
				request.setPropertyValue(param, config
						.getValueArray(currentIndex));
			}
			currentIndex++;
		}
	...
	/**
	 * Gets the TestStep with the appropriate parameters fuzzed with the next
	 * fuzz value Later we can make this more complex to use various encodings
	 * 
	 * @param testStep
	 *            The TestStep to fuzz
	 * @param params
	 *            the List of Parameters to fuzz
	 * 
	 */
	...
	/**
	 * This creates a fuzzer designed to test for a SQL injection This will be
	 * sufficient to test if the vulnerability exists, although not its extent,
	 * since deep testing that could be dangerous to the database.
	 * 
	 * @return
	 */
	public static Fuzzer getSQLFuzzer() {
		if (sqlFuzzer == null) {
			String[] values = { "' or '1'='1", "'--", "1'", "admin'--",
					"/*!10000%201/0%20*/", "/*!10000 1/0 */", "1/0",
					"'%20o/**/r%201/0%20--", "' o/**/r 1/0 --", ";",
					"'%20and%201=2%20--", "' and 1=2 --",
