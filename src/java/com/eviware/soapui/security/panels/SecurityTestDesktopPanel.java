...
public class SecurityTestDesktopPanel extends
		ModelItemDesktopPanel<SecurityTest> {
	...
	public SecurityTestDesktopPanel(SecurityTest securityTest) {
		...
		progressBarAdapter = new ProgressBarSecurityTestAdapter(progressBar,
				securityTest);
	}
	...
	private void buildUI() {
		...
		inspectorPanel
				.addInspector(new JComponentInspector<JComponent>(
						buildTestLog(), "Security Log",
		...
		if (StringUtils.hasContent(getModelItem().getDescription())
				&& getModelItem().getSettings().getBoolean(
	...
	private Component buildRunnerBar() {
		...
		// );
		progressBar = new JProgressBar(0, getModelItem().getTestCase()
				.getTestStepCount());
	...
	private JComponent buildContent() {
		...
		testStepListInspectorPanel = JInspectorPanelFactory.build(
				buildTestStepList(), SwingConstants.BOTTOM);
	...
	protected void addTabs(JTabbedPane tabs, JInspectorPanel inspectorPanel) {
		inspectorPanel.addInspector(new JFocusableComponentInspector<JPanel>(
				buildDescriptionPanel(), descriptionArea, "Description",
				"SecurityTest Description", true));
		inspectorPanel.addInspector(new JComponentInspector<JComponent>(
				buildPropertiesPanel(), "Properties",
				"SecurityTest level properties", true));
		inspectorPanel.addInspector(new GroovyEditorInspector(
				buildSetupScriptPanel(), "Setup Script",
				"Script to run before tunning a SecurityTest"));
		inspectorPanel.addInspector(new GroovyEditorInspector(
				buildTearDownScriptPanel(), "TearDown Script",
				"Script to run after a SecurityTest Run"));
	}

	protected GroovyEditorComponent buildTearDownScriptPanel() {
		tearDownGroovyEditor = new GroovyEditorComponent(
				new TearDownScriptGroovyEditorModel(), null);
		return tearDownGroovyEditor;
	}

	protected GroovyEditorComponent buildSetupScriptPanel() {
		setupGroovyEditor = new GroovyEditorComponent(
				new SetupScriptGroovyEditorModel(), null);
	...
	private Component buildToolbar() {
		...
		optionsButton = UISupport.createToolbarButton(SwingActionDelegate
				.createDelegate(SecurityTestOptionsAction.SOAPUI_ACTION_ID,
						getModelItem(), null, "/options.gif"));
		optionsButton.setText(null);
		cancelButton = UISupport.createToolbarButton(
				new CancelRunSecuritytestAction(), false);
		...
		setCredentialsButton = UISupport
				.createToolbarButton(new SetCredentialsAction(getModelItem()
						.getTestCase()));
		setEndpointButton = UISupport
				.createToolbarButton(new SetEndpointAction(getModelItem()
						.getTestCase()));
		...
		toolbar.add(UISupport.createToolbarButton(new ShowOnlineHelpAction(
				HelpUrls.TESTCASEEDITOR_HELP_URL)));
	...
	public class CancelRunSecuritytestAction extends AbstractAction {
		public CancelRunSecuritytestAction() {
			...
			putValue(Action.SHORT_DESCRIPTION,
					"Stops running this securitytest");
		}
	...
	public boolean onClose(boolean canCancel) {
		if (canCancel) {
			if (runner != null
					&& runner.getStatus() == TestCaseRunner.Status.RUNNING) {
				Boolean retval = UISupport.confirmOrCancel(
						"Cancel running SecurityTest?", "Cancel Run");
	...
	private class SetupScriptGroovyEditorModel extends
					...
					try {
						MockTestRunner mockTestRunner = new MockTestRunner(
								SecurityTestDesktopPanel.this.getModelItem()
										...
										.ensureGroovyLog());
						SecurityTestDesktopPanel.this.getModelItem()
								.getTestCase().runSetupScript(
										new MockTestRunContext(mockTestRunner,
												null), mockTestRunner);
					} catch (Exception e1) {
		...
		public SetupScriptGroovyEditorModel() {
			super(new String[] { "log", "testCase", "context", "testRunner" },
					SecurityTestDesktopPanel.this.getModelItem(), "Setup");
		}

		public String getScript() {
			return SecurityTestDesktopPanel.this.getModelItem()
					.getStartupScript();
		}
	...
	private class TearDownScriptGroovyEditorModel extends
		...
		public TearDownScriptGroovyEditorModel() {
			super(
					new String[] { "log", "securityTest", "context",
							"testRunner" }, SecurityTestDesktopPanel.this
		...
		public void setScript(String text) {
			SecurityTestDesktopPanel.this.getModelItem()
					.setTearDownScript(text);
		}
	}

	private class AddSecurityCheckAction extends AbstractAction implements
			Runnable {
		private final WsdlTestStepFactory factory;

		public AddSecurityCheckAction(WsdlTestStepFactory factory) {
			this.factory = factory;
			putValue(SMALL_ICON, UISupport.createImageIcon(factory
					.getTestStepIconPath()));
			putValue(SHORT_DESCRIPTION, "Create a new "
					+ factory.getTestStepName() + " TestStep");
		}
	...
	public static class ModelItemListDragAndDropable extends
			JListDragAndDropable<JList> {
		...
		public Component getRenderer(ModelItem modelItem) {
			return getList().getCellRenderer().getListCellRendererComponent(
					getList(), modelItem, getModelItemRow(modelItem), true,
					true);
		}

		@Override
		public void setDragInfo(String dropInfo) {
			super.setDragInfo(dropInfo == null || dropInfo.length() == 0 ? null
					: dropInfo);
		}
	}
