...
public class SecurityTestRunnerImpl extends
		AbstractTestRunner<SecurityTest, SecurityTestRunContext> implements
	...
	private List<TestStepResult> testStepResults = Collections
			.synchronizedList(new TreeList());
	...
	public TestStepResult runTestStep(TestStep testStep, boolean discard,
			boolean process) {
		...
		if (process && stepResult.getStatus() == TestStepStatus.FAILED) {
			if (getTestRunnable().getFailSecurityTestOnCheckErrors()) {
				...
				getRunContext().setProperty(
						TestCaseRunner.Status.class.getName(),
	...
	/**
	 * Clones original TestStep for security modification this does not alter
	 * the original test step
	 * 
	...
	private TestStep cloneForSecurityCheck(WsdlTestStep sourceTestStep) {
		...
		TestStepConfig testStepConfig = (TestStepConfig) sourceTestStep
				.getConfig().copy();
		WsdlTestStepFactory factory = WsdlTestStepRegistry.getInstance()
				.getFactory(testStepConfig.getType());
		if (factory != null) {
			clonedTestStep = factory.buildTestStep(securityTest.getTestCase(),
					testStepConfig, false);
			if (clonedTestStep instanceof Assertable) {
				for (TestAssertion assertion : ((Assertable) clonedTestStep)
						.getAssertionList()) {
	...
	public void internalRun(SecurityTestRunContext runContext) throws Exception {
		...
		HashMap<String, List<SecurityCheck>> secCheckMap = securityTest
				.getSecurityChecksMap();
		...
		if (testCase.getKeepSession()) {
			runContext.setProperty(SubmitContext.HTTP_STATE_PROPERTY,
					new HttpState());
		}
		...
		for (; initCount < testCase.getTestStepCount() && isRunning(); initCount++) {
			...
			try {
				...
				setStatus(Status.FAILED);
				SoapUI.logError(e);
				throw new Exception("Failed to prepare testStep ["
						+ testStep.getName() + "]; " + e.toString());
			}
		}
		...
		for (; isRunning() && currentStepIndex < testCase.getTestStepCount(); currentStepIndex++) {
			...
			securityTestStepListeners = securityTest
					.getTestStepRunListeners(currentStep);
			if (!currentStep.isDisabled()) {
				...
				for (int i = 0; i < securityTestStepListeners.length; i++) {
					securityTestStepListeners[i].beforeStep(this,
							getRunContext());
					if (!isRunning())
						return;
				}
				...
				if (secCheckMap.containsKey(currentStep.getId())) {
					List<SecurityCheck> testStepChecksList = secCheckMap
							.get(currentStep.getId());
					for (int i = 0; i < testStepChecksList.size(); i++) {
						...
						runTestStepSecurityCheck(runContext, currentStep,
								securityCheck);
					}
				}
				for (int i = 0; i < securityTestStepListeners.length; i++) {
					securityTestStepListeners[i].afterStep(this,
							getRunContext(), stepResult);
					if (!isRunning())
						return;
				}
				for (int i = 0; i < listeners.length; i++) {
	...
	public void runTestStepSecurityCheck(SecurityTestRunContext runContext,
			TestStep currentStep, SecurityCheck securityCheck) {
		if (securityCheck.acceptsTestStep(currentStep)) {
			...
			if (securityTest.getFailSecurityTestOnCheckErrors()
					&& result.getStatus() == SecurityCheckStatus.FAILED) {
				fail("Failing due to failed security check");
			}
			for (int j = 0; j < securityTestStepListeners.length; j++) {
				securityTestStepListeners[j].afterSecurityCheck(this,
						runContext, result);
			}
			for (int j = 0; j < seccheckListeners.length; j++) {
				seccheckListeners[j].afterSecurityCheck(this, runContext,
						result);
			}
	...
	protected void internalFinally(SecurityTestRunContext runContext) {
		...
		try {
			...
			securityTest.getSecurityTestLog().addEntry(
					new SecurityTestLogMessageEntry(" SecurityTest ended at "
