...
public class JSecurityTestRunLog extends JPanel {
	...
	private void buildUI() {
		logListModel = new SecurityTestLogModel();
		logListModel.setMaxSize((int) settings.getLong(OptionsForm.class
				.getName()
				+ "@max_rows", 1000));
	...
	protected void addToolbarButtons(JXToolBar toolbar) {
		...
		toolbar.addFixed(UISupport
				.createToolbarButton(new SetLogOptionsAction()));
	...
	public synchronized void updateSecurityTestStepResult(
			SecurityTestStepResult testStepResult, boolean hasChecksToProcess,
			boolean startStepLogEntryAdded) {
		logListModel.updateSecurityTestStepResult(testStepResult, errorsOnly,
				hasChecksToProcess, startStepLogEntryAdded);
		if (follow) {
			try {
				testLogList.ensureIndexIsVisible(logListModel.getSize() - 1);
			} catch (RuntimeException e) {
	...
	public synchronized void updateSecurityCheckResult(
	...
	public synchronized void addSecurityCheckRequestResult(
		...
		if (errorsOnly
				&& checkRequestResult.getStatus() != SecurityCheckRequestResult.ResultStatus.FAILED)
			return;
	...
	private class SetLogOptionsAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			optionsDialog.setIntValue(OptionsForm.MAXROWS, (int) settings
					.getLong(OptionsForm.class.getName() + "@max_rows", 1000));
			optionsDialog.setBooleanValue(OptionsForm.ERRORSONLY, settings
					.getBoolean(OptionsForm.class.getName() + "@errors_only"));
			optionsDialog.setBooleanValue(OptionsForm.FOLLOW, follow);

			if (optionsDialog.show()) {
				int maxRows = optionsDialog.getIntValue(OptionsForm.MAXROWS,
						1000);
				logListModel.setMaxSize(maxRows);
				settings.setLong(OptionsForm.class.getName() + "@max_rows",
						maxRows);
				errorsOnly = optionsDialog
						.getBooleanValue(OptionsForm.ERRORSONLY);
				settings.setBoolean(OptionsForm.class.getName()
	...
	private class ClearLogAction extends AbstractAction {
		public ClearLogAction() {
			putValue(Action.SMALL_ICON, UISupport
					.createImageIcon("/clear_loadtest.gif"));
	...
	private final class SecurityTestLogCellRenderer extends JLabel implements
			ListCellRenderer {
		...
		public SecurityTestLogCellRenderer() {
			...
			hyperlinkLabel.setBorder(BorderFactory
					.createEmptyBorder(0, 4, 3, 3));
		}
		...
		public Component getListCellRendererComponent(JList list, Object value,
			...
			if (result != null) {
				if (result.getResultType().equals(
						SecurityCheckRequestResult.TYPE)) {
					...
					hyperlinkLabel.setBorder(BorderFactory.createEmptyBorder(0,
							24, 3, 3));
				} else if (result.getResultType().equals(
		...
		private void setStatusIcon(SecurityResult securityResult) {
			if (securityResult.getLogIconStatus() == ResultStatus.OK) {
				hyperlinkLabel.setIcon(UISupport
						.createImageIcon("/valid_assertion.gif"));
			} else if (securityResult.getLogIconStatus() == ResultStatus.FAILED) {
				hyperlinkLabel.setIcon(UISupport
						.createImageIcon("/failed_assertion.gif"));
				...
				hyperlinkLabel.setIcon(UISupport
						.createImageIcon("/missing_assertion.gif"));
			} else {
				hyperlinkLabel.setIcon(UISupport
						.createImageIcon("/unknown_assertion.gif"));
			}
		}
	}
