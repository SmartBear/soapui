...
//		project = new WsdlProject( projectFile );
		...
		// start by listening to all testcases.. (since one testcase can call another)
		for( int c = 0; c < project.getTestSuiteCount(); c++ )
		{
			...
			for( int i = 0; i < suite.getTestCaseCount(); i++ )
			{
				...
				if (junitReport)
					suite.getTestCaseAt( i ).addTestRunListener(reportCollector);
			}
		}
		...
		if( result.getStatus() == TestStepStatus.FAILED || exportAll )
		{
			try
			{
				...
				String nameBase = StringUtils.createFileName( tc.getTestSuite().getName(), '_' ) + "-" +
											  ...
											  count.longValue() + "-" + result.getStatus();
