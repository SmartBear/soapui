...
public class WorkspaceImpl extends AbstractModelItem implements Workspace
{
   ...
   public void loadWorkspace( File file ) throws XmlException, IOException
   {
      if( file.exists() )
      {
         ...
         settings = new XmlBeansSettingsImpl( this, SoapUI.getSettings(), workspaceConfig
                 .getSoapuiWorkspace().getSettings() );
...
//								wsc.getStatus() != Status.CLOSED && wsc.getType() != Type.REMOTE, wsc.getName(), null);
            ...
            {
               UISupport.showErrorMessage( messages.get( "FailedToLoadProjectInWorkspace", str )
                       + e.getMessage() );
   ...
   public void removeProject( Project project )
   {
      ...
      if( ix == -1 )
         throw new RuntimeException( "Project [" + project.getName()
                 + "] not available in workspace for removal" );
...
//			workspaceConfig.getSoapuiWorkspace().removeProject( ix );
   ...
   {
      WorkspaceListener[] listenerArray = listeners
              .toArray( new WorkspaceListener[listeners.size()] );
...
//		WsdlProject project = new WsdlProject( url, this, false );
   ...
   public void closeProject( Project project )
   {
      ...
      if( ix == -1 )
         throw new RuntimeException( "Project [" + project.getName()
                 + "] not available in workspace for close" );
