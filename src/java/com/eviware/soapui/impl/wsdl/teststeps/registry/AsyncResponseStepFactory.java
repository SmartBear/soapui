...
public class AsyncResponseStepFactory extends WsdlTestStepFactory
{
	...
	public AsyncResponseStepFactory()
	{
		super(ASYNC_RESPONSE_TYPE, "Asynchronous Response",
				"Responds to an asynchronous call based on XPath expressions", 
				"/asyncResponseStep.gif");
	}

    public WsdlTestStep buildTestStep(
    		WsdlTestCase testcase, TestStepConfig config, boolean flag)
    {
        ...
        if (form == null)
        {
            ...
            form.getFormField(CreateForm.INTERFACE).addFormFieldListener(new XFormFieldListener() 
            {
                public void valueChanged(XFormField sourceField, String newValue, String oldValue)
                {
                    ...
                    form.setOptions(CreateForm.OPERATION, 
                    		(new ModelItemNames(wsdlinterface.getOperationList())).getNames());
                }
    ...
    private static TestStepConfig createConfig(WsdlProject wsdlProject, String name)
    {
        ...
        if (form.getBooleanValue(CreateForm.CREATE_RESPONSE))
        {
            WsdlInterface iface = (WsdlInterface) project.getInterfaceByName(config.getInterface());
				String content = iface.getOperationByName( config.getOperation()).createResponse(project.getSettings().getBoolean(
    ...
    public static TestStepConfig createConfig(
    		WsdlOperation operation, WsdlRequest request, boolean flag)
    {
        if (flag)
        {
            createForm();
            ...
            return createConfig(operation.getInterface().getProject(), 
            		(new StringBuilder()).append(request.getName()).append(" Response").toString());
        }
        ...
        {
            CompressedStringSupport.setString(mockResponseConfig.getResponseContent(), request.getResponse().getContentAsString());
        }
...
//            form.setValue(CreateForm.REQUEST_QUERY, response.getMockOperation().getMockService().getRequestQuery());
//            form.setValue(CreateForm.MATCHING_VALUE, response.getMockOperation().getMockService().getMatchingValue());
            
            return createConfig(wsdloperation.getInterface().getProject(), 
            		(new StringBuilder()).append(
            				response.getMockOperation().getName()).append(
            						" - ").append(response.getName()).toString());
        }
    }
