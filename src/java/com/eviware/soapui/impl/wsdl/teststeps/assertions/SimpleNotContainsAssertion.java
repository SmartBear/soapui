...
/**
 * Assertion that checks for the non-existence of a specified text token in the associated 
 * WsdlTestRequests response message
 * 
 * @author Ole.Matzura
 */
   ...
   public String internalAssertResponse(WsdlMessageExchange messageExchange, SubmitContext context) throws AssertionException
   {
	...
	private String assertContent( SubmitContext context, String content, String type ) throws AssertionException
	{
      ...
      if( replToken.length() > 0 )
      {
      	...
      	{
	      	ix = ignoreCase ?
	      				content.toUpperCase().indexOf( replToken.toUpperCase() ) : content.indexOf( replToken );
      	}
