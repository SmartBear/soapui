...
public class ChangeOperationAction extends AbstractSoapUIAction<WsdlTestRequestStep>
{
		...
		if( dialog == null )
   	{
		...
		if( dialog.show() )
		{
			...
			if( dialog.getBooleanValue( Form.RECREATE_REQUEST ))
			{
	         ...
	         {
	         	req = SoapUtils.transferSoapHeaders(request.getRequestContent(), req, operation.getInterface().getSoapVersion());
	...
	@AForm( description = "Specify Interface/Operation for TestRequest", name = "Change Operation",
				helpUrl=HelpUrls.CHANGEOPERATION_HELP_URL, icon=UISupport.TOOL_ICON_PATH)
