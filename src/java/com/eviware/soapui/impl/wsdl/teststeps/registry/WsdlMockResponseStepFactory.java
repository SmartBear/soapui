...
public class WsdlMockResponseStepFactory extends WsdlTestStepFactory
{
	...
	@AForm(description = "Secify options for new MockResponse step", name = "New MockResponse Step",
				helpUrl= HelpUrls.CREATEMOCKRESPONSESTEP_HELP_URL, icon=UISupport.OPTIONS_ICON_PATH)
	private class CreateForm
	{
		...
		@AField(description = "Specifies if a mock response is to be created from the schema",
					name = "Create Response", type = AFieldType.BOOLEAN )
	...
	public static TestStepConfig createNewTestStep( WsdlMockResponse mockResponse)
	{
		...
		return createFromDialog( operation.getInterface().getProject(),
					mockResponse.getMockOperation().getName() + " - " + mockResponse.getName() );
	}
