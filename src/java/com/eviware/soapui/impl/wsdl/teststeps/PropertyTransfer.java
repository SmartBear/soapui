...
public class PropertyTransfer implements PropertyChangeNotifier
{
	...
	// create local copies since a deleted/changed valuetransfer can be referred to from a result 
	...
	public String [] transferProperties(SubmitContext context) throws PropertyTransferException
	{
		...
		{
			throw new PropertyTransferException( e.getMessage(), getSourceStepName(), sourceProperty, 
					getTargetStepName(), targetProperty );
		}		
	...
	protected String[] transferXPathToXml( TestProperty sourceProperty, TestProperty targetProperty, SubmitContext context) throws XmlException, Exception
	{
	...
	private String setNodeValue(String value, Node node) throws Exception
	{
		...
		{
			node = ((DocumentFragment) node ).getFirstChild();
			if( node != null )
	...
	protected String transferXPathToString( TestProperty sourceProperty, TestProperty targetProperty, SubmitContext context) throws XmlException, Exception
	{
	...
	private String transferXmlValue(XmlCursor source, XmlCursor dest) throws Exception
	{
				...
				(sourceNodeType == Node.ATTRIBUTE_NODE && destNodeType == Node.TEXT_NODE) )
		{
			...
			if( !getIgnoreEmpty() || (value != null && value.length() > 0) )
				...
				destNodeType == Node.ATTRIBUTE_NODE || destNodeType == Node.TEXT_NODE )
		{
			value = XmlUtils.getElementText( (Element) sourceNode );
			if( !getIgnoreEmpty() || (value != null && value.length() > 0) )
				destNode.setNodeValue( value );
		}
		else if( destNodeType == Node.ELEMENT_NODE && 
				sourceNodeType == Node.ATTRIBUTE_NODE || sourceNodeType == Node.TEXT_NODE )
		{
	...
	public void setSourcePropertyName( String name )
	{
		...
		// check for change
		if( (name == null && old == null) || (name != null && old != null && name.equals( old ))) return;
		...
		currentSourceProperty = sourceStep2 != null && sourceType != null ? 
				sourceStep2.getProperty( sourceType ) : null;
	...
	public void setTargetPropertyName( String name )
	{
		...
		currentTargetProperty = targetStep2 != null && targetType != null ? 
				targetStep2.getProperty( targetType ) : null;
	...
	private class InternalTestPropertyListener extends TestPropertyListenerAdapter
	{
		...
		public void propertyRemoved(String name)
		{
			...
			{
				log.warn( "source property for transfer [" + getName() + "] in teststep [" + 
						testStep.getName() + "/" + testStep.getTestCase().getName() + "/" + testStep.getTestCase().getTestSuite().getName() + 
			...
			{
				log.warn( "target property for transfer [" + getName() + "] in teststep [" + 
						testStep.getName() + "/" + testStep.getTestCase().getName() + "/" + testStep.getTestCase().getTestSuite().getName() + 
						"] set to null, was [" + name + "]" );  
