...
//   		existing.put( wsdlUrl, null );
      ...
      try
      {
         ...
         {
            // set targetNamespace (this happens if we are following an include statement)
            if( tns != null )
            {
   ...
   private static void selectDefinitionParts( String wsdlUrl, Map<String, XmlObject> existing, SchemaLoader loader, XmlObject xmlObject, String path )
           throws Exception
   {
      ...
      if( containerField instanceof SchemaLocalElement )
      {
         ...
         if( annotation != null )
         {
            ...
            if( userInformation != null && userInformation.length > 0 )
            {
               ...
               result = xmlObject.xmlText(); // XmlUtils.getElementText( ( Element ) userInformation[0].getDomNode());
            }
         }
      }
