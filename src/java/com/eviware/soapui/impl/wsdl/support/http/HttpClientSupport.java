	...
	private static class Helper
	{
		...
		GeneralSecurityException
		{
			initKeyMaterial( easySSL );
			 ...
			 /*
	       Commented out for now - EasySSLProtocolSocketFactory already
	       trusts everything!  Below is some code that might work for when
	       SoapUI moves away from "EasySSLProtocolSocketFactory".                     

	    String trustStore = settings.getString( SSLSettings.TRUSTSTORE, null );
	    trustStore = trustStore != null ? trustStore.trim() : "";
	    pass = settings.getString( SSLSettings.TRUSTSTORE_PASSWORD, "" );
	    ...
	    if ( !"".equals( trustStore ) ) {
	        ...
	        if ( f.exists() ) {
	            ...
	            try
	            {
	                tm = new TrustMaterial( trustStore, pwd );
	            }
	            catch ( GeneralSecurityException gse )
	            {
	                String trimmedPass = pass.trim();
	                if ( "".equals( trimmedPass ) )
	                {
	                    // If the password is all spaces, then we'll allow
	                    // loading of the TrustMaterial without a password.
	                    tm = new TrustMaterial( trustStore );
	                }
	                else
	                {
	                    log.error( "Failed to load TrustMaterial: " + gse );
	                }
	            }
	            if ( tm != null )
	            {
	                easySSL.setTrustMaterial( tm );
	                log.info( "Added TrustStore from file [" + trustStore + "]" );
	            }
	        } else {
...
*/
	...
	public static void applyHttpSettings( HttpMethod httpMethod, Settings settings )
	{
		...
		// timeout?
		long timeout = settings.getLong( HttpSettings.SOCKET_TIMEOUT,
					HttpSettings.DEFAULT_SOCKET_TIMEOUT );
