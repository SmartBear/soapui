 ...
 *
 * @author Ole.Matzura
 * @todo statistics should be calculated first after all threads have been started..
 */
...
public class WsdlLoadTestRunner implements LoadTestRunner
{
               ...
               public Object construct( XProgressMonitor monitor )
               {
                  ...
                  while( !testCases.isEmpty() && !canceled )
                  {
                     ...
                     if( status != Status.RUNNING || getProgress() >= 1 ||
                             ( loadTest.getLimitType() == LoadTestLimitTypesConfig.COUNT &&
                                     runners.size() >= loadTest.getTestLimit() ) )
                        break;
   ...
   private TestCaseRunner startTestCase( WsdlTestCase testCase )
   {
      ...
      Thread thread = new Thread( threadGroup, testCaseRunner,
              testCase.getName() + " - " + loadTest.getName() + " - ThreadIndex " + testCaseRunner.threadIndex );
      thread.start();
   ...
   private synchronized boolean afterRun( TestCaseRunner runner )
   {
      ...
      if( loadTest.getTestLimit() < 1 ) return true;
   ...
   /**
    * Creates a copy of the underlying WsdlTestCase with all LoadTests removed and configured for LoadTesting
    */
   ...
   private class InternalTestRunListener implements TestRunListener
   {
      public void beforeRun( TestRunner testRunner, TestRunContext runContext )
      {
         ...
         else for( LoadTestRunListener listener : loadTest.getLoadTestRunListeners() )
         {
      ...
      public void beforeStep( TestRunner testRunner, TestRunContext runContext )
      {
         ...
         {
            listener.beforeTestStep( WsdlLoadTestRunner.this, context, testRunner, runContext, runContext.getCurrentStep() );
         }
      }
