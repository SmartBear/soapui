...
public final class LoadTestStatistics extends AbstractTableModel implements Runnable
{
	...
	public String getColumnName(int columnIndex)
	{
		...
		{
			case 0 : return " ";
			case 1 : return "Test Step";
			case 2 : return Statistic.MININMUM.getName();
			case 3 : return Statistic.MAXIMUM.getName();
			case 4 : return Statistic.AVERAGE.getName();
			case 5 : return Statistic.LAST.getName();
			case 6 : return Statistic.COUNT.getName();
			case 7 : return Statistic.TPS.getName();
			case 8 : return Statistic.BYTES.getName();
			case 9 : return Statistic.BPS.getName();
			case 10 : return Statistic.ERRORS.getName();
		}
		return null;
	}
	...
	public Object getValueAt(int rowIndex, int columnIndex)
	{
		...
		switch( columnIndex )
		{
			case 0 : return rowIndex == testCase.getTestStepCount() ? null : 
				 ColorPalette.getColor( testCase.getTestStepAt( rowIndex ) );
			case 1 :
			{
			...
			case 4 :
			case 7 : return new Float( (float)data[rowIndex][columnIndex-2]/100 );
			default :
			{
	...
	private synchronized void addSamples( SamplesHolder holder )
	{
		...
		{
			adding = false;
			throw new RuntimeException( "Unexpected number of samples: " + holder.samples.length + 
					", exptected " + (totalIndex) );
		}
			...
			if( holder.sampleCounts[c] > 0 )
			{
				...
				if( holder.complete != loadTest.getUpdateStatisticsPerTestStep())
				{
					...
					if( timePassed > 0 )
					{
						...
						{
							data[c][TPS_COLUMN] = (long) (data[c][AVG_COLUMN] > 0 ? 
									(100000F/average)*threadCount : 0);
		...
		if( holder.complete )
		{
			...
			if( timePassed > 0 )
			{
				...
				{
					data[totalIndex][TPS_COLUMN] = (long) (data[totalIndex][AVG_COLUMN] > 0 ? 
							(10000000F/data[totalIndex][AVG_COLUMN])*threadCount : 0);
					
					long avgBytes = data[totalIndex][CNT_COLUMN] == 0 ? 0 : 
						data[totalIndex][BYTES_COLUMN] / data[totalIndex][CNT_COLUMN];
	...
	public synchronized StringList [] getSnapshot()
	{
		...
		for( int c = 0; c < clone.length; c++ )
		{
			...
			for( int columnIndex = 2; columnIndex < getColumnCount(); columnIndex ++ )
			{
				...
				{
					case 4 :
					case 7 : values.add( String.valueOf( (float)data[c][columnIndex-2]/100 ) ); break;
					default : values.add( String.valueOf( data[c][columnIndex-2] ));
				}
			}
