...
public class LoadTestAssertionRegistry
{
   private static LoadTestAssertionRegistry instance;
	   ...
	   try
		{
			...
			Constructor<? extends AbstractLoadTestAssertion> ctor = clazz
					.getConstructor(new Class[] { LoadTestAssertionConfig.class, WsdlLoadTest.class  });
	...
	{
      return  getInstance().availableAssertions.keySet().toArray( 
      		new String[getInstance().availableAssertions.size()] );
	}
