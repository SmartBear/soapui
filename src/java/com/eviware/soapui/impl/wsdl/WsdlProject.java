...
public class WsdlProject extends AbstractTestPropertyHolderWsdlModelItem<ProjectConfig> implements Project, PropertyExpansionContainer
{
	...
	public boolean save() throws IOException 
   {
      ...
      if( path == null || isRemote())
      {
         ...
         {
         	file = UISupport.getFileDialogs().saveAs(this, "Save project " + getName(), ".xml",
      ...
      // check modified 
		if( projectFile.exists() && lastModified != 0 && lastModified < projectFile.lastModified() )
		{
			if( !UISupport.confirm( 
						"Project file for [" + getName() + "] has been modified externally, overwrite?", 
						"Save Project" ))
				return false;
		}
	...
	private boolean saveIn(File projectFile) throws IOException 
	{
   	...
   	lastModified = projectFile.lastModified();
		log.info( "Saved project [" + getName() + "] to [" + projectFile.getAbsolutePath() + " - " +
				size + " bytes" );
		setProjectRoot(path);
		return true;
}
   ...
   public boolean hasNature(String natureId)
   {
	...
	{
		testSuite.beforeSave();
		TestSuiteConfig testSuiteConfig = ( TestSuiteConfig ) getConfig().addNewTestSuite().set( testSuite.getConfig().copy() );
