...
public class AddRequestAsMockResponseStepAction extends AbstractAddToTestCaseAction<WsdlRequest>
{
	...
	protected boolean addMockResponse(WsdlTestCase testCase, WsdlRequest request)
	{
		...
		dialog.getFormField( Form.CLOSE_REQUEST ).setEnabled( 
					desktop != null && desktop.hasDesktopPanel( request ));
	...
	@AForm( name="Add MockResponse to TestCase", description="Options for adding this requests response to a TestCase",
				helpUrl= HelpUrls.ADDREQUESTASMOCKRESPONSESTEP_HELP_URL, icon=UISupport.OPTIONS_ICON_PATH )
