...
public class UpdateInterfaceAction extends AbstractSoapUIAction<WsdlInterface>
{
   ...
   protected void afterUpdate(WsdlInterface iface) throws Exception
   {
      if( dialog.getBooleanValue( Form.RECREATE_REQUESTS ))
      {
      	...
      	{
      		UISupport.showDesktopPanel( new ModelItemListDesktopPanel( "Updated Requests/TestRequests", 
      					"The following Request/TestRequests where updated", updated.toArray( new ModelItem[updated.size()] )));
      	}
      }
      else
      {
			...
			for( Request request : requests )
			{
				...
				if( !req.equals( requestContent ) )
				{
					if( !XmlUtils.prettyPrintXml( req ).equals( XmlUtils.prettyPrintXml( requestContent )))
					{
						if( createBackups )
						{
							WsdlRequest backupRequest = operation.addNewRequest( 
										"Backup of [" + request.getName() + "]" );
   ...
   @AForm(description = "Specify Update Definition options", name = "Update Definition", 
   			helpUrl=HelpUrls.UPDATE_INTERFACE_HELP_URL,  icon=UISupport.TOOL_ICON_PATH  )
