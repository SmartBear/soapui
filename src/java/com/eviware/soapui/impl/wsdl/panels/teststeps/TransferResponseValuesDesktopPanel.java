...
public class TransferResponseValuesDesktopPanel extends ModelItemDesktopPanel<TransferResponseValuesTestStep>
{
	...
	protected JXToolBar createTargetToolbar()
	{
		...
		targetStepCombo.addItemListener( 
				new StepComboItemListener( targetPropertyCombo, targetStepPropertiesListener ) );
	...
	/**
	 * Listen for testStep property changes and update properties combo accordingly
	 */
	...
	private final class TestStepPropertiesListener implements TestPropertyListener
	{
		...
		{
			TestProperty property = combo == targetPropertyCombo ?
				getCurrentTransfer().getTargetStep().getProperty( name ) :
		...
		{
			if( ((TestProperty)combo.getSelectedItem()).getName().equals( name ))
	...
	/**
	 * Listen for teststep changes and update source/target step combos accordingly
	 */
	...
	/**
	 * Listen to selection changes in transfer list and update controls accordingly
	 */
	...
	/**
	 * Listen to property changes and update UI objects. These may have been triggered by UI so first check
	 * for actual difference so we dont end up in loop.
	 */
	...
	private class TransfersTableModel extends AbstractTableModel
	{
		...
		public String getColumnName(int column)
		{
			...
			case 1 : return "Transfer Name";
			case 2 : return "Transferred Values";
			}
		...
		public synchronized Object getValueAt(int rowIndex, int columnIndex)
		{
			...
			if( result != null )
			{
				switch( columnIndex )
				{
					case 0 : return new Date( result.getTimeStamp() ).toString();
					case 1 : return result.getTransferAt( rowIndex-sum ).getName();
					case 2 : return Arrays.toString( result.getTransferredValuesAt( rowIndex-sum ) );
				}
			}
