...
public class WsdlTestCaseDesktopPanel extends ModelItemDesktopPanel<WsdlTestCase>
{
	...
	public WsdlTestCaseDesktopPanel( WsdlTestCase testCase )
	{
		...
		SoapUIDragAndDropHandler dragAndDropHandler = new SoapUIDragAndDropHandler( 
					new ModelItemListDragAndDropable( getTestStepList().getTestStepList(), testCase ), DropType.BEFORE_AND_AFTER );
		
		dragSource.createDefaultDragGestureRecognizer( getTestStepList().getTestStepList(), DnDConstants.ACTION_COPY_OR_MOVE,
					dragAndDropHandler );
	}
	...
	private JComponent buildContent()
	{
		...
		JInspectorPanel inspectorPanel = new JInspectorPanel( buildTestStepList(),
					SwingConstants.BOTTOM);
		
		tabs.addTab( "TestSteps", inspectorPanel );
	...
	protected void addTabs( JTabbedPane tabs, JInspectorPanel inspectorPanel )
	{
		inspectorPanel.addInspector( new JFocusableComponentInspector<JPanel>( buildDescriptionPanel(), 
					descriptionArea, "Description", "TestCase Description", true ) );
	...
	public class InternalTestRunListener extends TestRunListenerAdapter
	{
		...
		public void afterRun( TestRunner testRunner, TestRunContext runContext )
		{
			...
			else if( testRunner.getStatus() == TestRunner.Status.FAILED )
			{
				...
				testCaseLog.addText( "TestCase failed [" + msg + "], time taken = "
							+ wsdlRunner.getTimeTaken() );
			}
			else 
   ...
   protected void beforeRun()
   {
	...
	private class AddWsdlTestStepAction extends AbstractAction
	{
		...
		public void actionPerformed( ActionEvent e )
		{
			...
			String name = UISupport.prompt( "Specify name for new step", ix == -1 ? "Add Step" : "Insert Step", factory.getTestStepName());
			if( name != null )
			{
