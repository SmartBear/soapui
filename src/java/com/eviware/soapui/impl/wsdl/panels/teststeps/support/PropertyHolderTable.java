...
//			setHorizontalScrollEnabled(true);
	...
	private class MovePropertyUpAction extends AbstractAction
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			if( ix != -1 )
			{
				((MutableTestPropertyHolder)holder).moveProperty(
					holder.getPropertyAt(ix).getName(), ix-1 );
				...
				if( flavor.isMimeTypeEqual( DataFlavor.javaJVMLocalObjectMimeType ) )
				{
					try
					{
						...
						if( modelItem instanceof PropertyModelItem && 
									((PropertyModelItem)modelItem).getProperty().getModelItem() != getHolder().getModelItem() )
						{
