...
//		setEnabled( request != null && request.getMockResult() != null );
	...
	private File buildConfig(File reportFile, File logFile, WsdlMockResponse modelItem) throws IOException
	{
		...
		results.setType( 
				AssertionResults.Type.Enum.forString( 
						settings.getString( WSISettings.RESULTS_TYPE, AssertionResults.Type.ONLY_FAILED.toString() )));
	...
	private void addMonitorConfig(Log log) throws Exception
	{
		...
		redirect.setListenPort( 9999 );
		redirect.setMaxConnections( 10 );
		redirect.setReadTimeoutSeconds( 10 );
		
		URL endpoint = new URL( modelItem.getEndpoint());
		if( endpoint.getPort() > 0 )
			redirect.setSchemeAndHostPort(  endpoint.getHost() + ":" + endpoint.getPort());
		else
		...
		protected void afterRun( int exitCode, RunnerContext context )
		{
			try
			{
               ...
               UISupport.showDesktopPanel( 
							new DefaultDesktopPanel( "WS-I Report", "WS-I Report for validation of messages in MockResponse [" + modelItem.getName() + "]", panel ));
				}
			}
			catch( Exception e )
			{
