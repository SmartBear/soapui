 ...
 * and displays these in seperate tabs
 * 
 * @author Ole.Matzura
 */
...
public class WsdlInterfaceDesktopPanel extends ModelItemDesktopPanel<WsdlInterface>
{
	...
	private JComponent buildWsdlContentTab()
   {
      ...
      rootNode = new DefaultMutableTreeNode( iface.getName() );
   	...
   	JScrollPane scrollPane = new JScrollPane( tree );
		JSplitPane split = UISupport.createHorizontalSplit( scrollPane, 
		      	      	  UISupport.createTabPanel( partTabs, true ));
		...
		try
		{
        	 if( iface.getWsdlContext().loadIfNecessary())
        	 {
	          XProgressDialog progressDialog = UISupport.getDialogs().createProgressDialog(
	       			"Loading Defintion", 3, "Initializing definition..", true );
	...
	private Component buildToolbar()
	{
		...
		JButton button = UISupport.createToolbarButton( SwingActionDelegate.createDelegate( UpdateInterfaceAction.SOAPUI_ACTION_ID, getModelItem(),
					null, "/updateDefinition.gif" ));
		button.setText( null );
	...
	private class Loader implements Worker
   {
      ...
      private void addTab(String url, XmlObject xmlObject) throws Exception
   	{
		...
		private void initInspectionTree( XmlObject xmlObject, JXEditTextArea inputArea )
		{
			...
			mapTreeItems( xmlObject, treeRoot, false, tabCount, "Complex Types", 
						"declare namespace xs='http://www.w3.org/2001/XMLSchema';//xs:complexType[@name!='']", "@name", true, null );

			mapTreeItems( xmlObject, treeRoot, false, tabCount, "Simple Types", 
						"declare namespace xs='http://www.w3.org/2001/XMLSchema';//xs:simpleType[@name!='']", "@name", true, null );
			...
			{
				mapTreeItems( ((InspectItem)treeNode.getUserObject()).item, treeNode, false, 
							tabCount, null, "declare namespace wsdl='http://schemas.xmlsoap.org/wsdl/';wsdl:part", 
			...
			{
				List<DefaultMutableTreeNode> operationNodes = mapTreeItems( ((InspectItem)treeNode.getUserObject()).item, treeNode, false, 
							tabCount, null, "declare namespace wsdl='http://schemas.xmlsoap.org/wsdl/';wsdl:operation", "@name", true, null );
				...
				for( DefaultMutableTreeNode treeNode2 : operationNodes )
				{
					mapTreeItems( ((InspectItem)treeNode2.getUserObject()).item, treeNode2, false, tabCount, null, 
								"declare namespace wsdl='http://schemas.xmlsoap.org/wsdl/';wsdl:*", "concat( @name, ' [', local-name(), '], message=[', @message, ']' )", false,
								new MessageSelector());
				}
			}
			
			List<DefaultMutableTreeNode> bindings = mapTreeItems( xmlObject, treeRoot, false, tabCount, "Bindings", 
						"declare namespace wsdl='http://schemas.xmlsoap.org/wsdl/';//wsdl:binding", 
						"declare namespace wsdlsoap='http://schemas.xmlsoap.org/wsdl/soap/';concat( @name, ' [style=', wsdlsoap:binding[1]/@style, ']' )", true, null );
			
			for( DefaultMutableTreeNode treeNode : bindings )
			{
				List<DefaultMutableTreeNode> operationNodes = mapTreeItems( ((InspectItem)treeNode.getUserObject()).item, treeNode, false, 
							tabCount, null, "declare namespace wsdl='http://schemas.xmlsoap.org/wsdl/';wsdl:operation", 
				...
				{
					mapTreeItems( ((InspectItem)treeNode2.getUserObject()).item, treeNode2, false, tabCount, null, 
								"declare namespace wsdl='http://schemas.xmlsoap.org/wsdl/';wsdl:*", "concat( @name, ' [', local-name(), ']' )", false,
			...
			for( DefaultMutableTreeNode treeNode : services )
			{
				mapTreeItems( ((InspectItem)treeNode.getUserObject()).item, treeNode, false, tabCount, null, 
							"declare namespace wsdl='http://schemas.xmlsoap.org/wsdl/';wsdl:port", "concat( 'port: name=[', @name, '] binding=[', @binding, ']' )", true, 
							new PortSelector() );
			}
	...
	public List<DefaultMutableTreeNode> mapTreeItems( XmlObject xmlObject, DefaultMutableTreeNode treeRoot, boolean createEmpty, 
				int tabIndex, String groupName, String query, String nameQuery, boolean sort, NodeSelector selector )
	{
	...
	private class OperationsTableModel extends AbstractTableModel
	{
		...
		public Object getValueAt( int rowIndex, int columnIndex )
		{
			...
			switch( columnIndex )
			{
			case 0 : return operation.getName();
			case 1 : 
			{
			...
			case 3 : return operation.getAction();
			case 2 : return Boolean.valueOf( operation.isUnidirectional() );
			}
