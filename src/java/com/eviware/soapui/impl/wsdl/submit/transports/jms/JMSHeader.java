...
public class JMSHeader
{
	...
	{
		JMSConfig jmsConfig = ((WsdlRequest) request).getJMSConfig();
		try
		{
			// JMSCORRELATIONID
...
//		StringToStringMap headersMap = temp.getRequestHeaders();
	...
	// public void setMessageHeaders(Message message, Request request, Hermes
	// hermes)
	// {
	// AbstractHttpRequest temp = (AbstractHttpRequest) request;
	// StringToStringMap headersMap = temp.getRequestHeaders();
	...
	// {
	// setTimeTolive(Long.parseLong(PropertyExpander.expandProperties(headersMap.get(TIMETOLIVE,
	// "0"))));
	// }
	// else
	// {
	...
	// for (String key : keys)
	// {
	// if (!key.equals(JMSCORRELATIONID) && !key.equals(JMSREPLYTO) &&
	// !key.equals(TIMETOLIVE)
	// && !key.equals(JMSTYPE) && !key.equals(JMSPRIORITY) &&
	// !key.equals(JMSDELIVERYMODE))
	// {
	// message.setStringProperty(key,
	// PropertyExpander.expandProperties(headersMap.get(key)));
	// }
	// }
	// }
	// catch (NamingException e)
	// {
	// SoapUI.logError(e, "Message header JMSReplyTo = "
	// + PropertyExpander.expandProperties(headersMap.get(JMSREPLYTO)) +
