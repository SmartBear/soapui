		...
		try 
		{
			...
			for ( currentStepIndex = 0; status == Status.RUNNING && currentStepIndex < testCase.getTestStepCount(); currentStepIndex++) 
			{
				...
				if( !currentStep.isDisabled() )
				{
					...
					TestStepResult stepResult = currentStep.run( this, runContext );
					...
					// discard?
					if( stepResult.getStatus() == TestStepStatus.OK && testCase.getDiscardOkResults() && 
						 !stepResult.isDiscarded()	)
					{
						stepResult.discard();
					}
	
					if( stepResult.getStatus() == TestStepStatus.FAILED )
					{
