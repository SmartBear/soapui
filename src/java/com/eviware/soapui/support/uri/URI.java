...
public class URI implements Cloneable, Comparable, Serializable
{
	 ...
	 * 
	 * @param original
	 *           the string to be represented to URI character sequence It is one
	 *           of absoluteURI and relativeURI.
	 * @param charset
	 *           the charset string to do escape encoding
	 * @throws URIException
	 *            If the URI cannot be created.
			...
			throws URIException
	{
		...
		this( scheme, ( host == null ) ? null : ( ( userinfo != null ) ? userinfo + '@' : "" ) + host
	...
	/**
	 * The default charset of the document. RFC 2277, 2396 The platform's charset
	 * is used for the document by default.
	 */
	 ...
	 * performed by maintaining a mapping table that includes the local character
	 * set code and the corresponding UCS code. The next step is to convert the
	 * UCS character code to the UTF-8 encoding.
	 * <p>
	 ...
	 * 
	 * This is a two mapping, one from URI characters to octets, and subsequently
	 ...
	 * Parse a URI reference as a <code>String</code> with the character encoding
	 * of the local system or the document.
	 * <p>
	...
	protected void parseUriReference( String original, boolean escaped ) throws URIException
	{
		...
		/*
		 * The length of the string sequence of characters. It may not be equal to
		...
		if( 0 <= at && at + 1 <= length && tmp.charAt( at ) == '#' )
		{
			...
			{
				_fragment = ( escaped ) ? tmp.substring( at + 1 ).toCharArray() : encode( tmp.substring( at + 1 ),
						allowed_fragment, charset );
			}
		}
	...
	protected void parseAuthority( String original, boolean escaped ) throws URIException
	{
		...
		{ // neither -1 and 0
			...
			_userinfo = ( escaped ) ? original.substring( 0, next ).toCharArray() : encode( original.substring( 0, next ),
					allowed_userinfo, charset );
			from = next + 1;
		}
		...
		if( next >= from )
		{
			...
			// In IPv6reference, '[', ']' should be excluded
			_host = ( escaped ) ? original.substring( from, next ).toCharArray() : encode(
					original.substring( from, next ), allowed_IPv6reference, charset );
			// Set flag
	 ...
	 * charset, or provide a way to indicate the charset used.
	 * <p>
	 * To work globally either requires support of a number of character sets and
	 * to be able to convert between them, or the use of a single preferred
	 ...
	 * namespace is a collection, the slash mark ('/') should be ended with at
	 * the last character of the path string.
	 * 
	 * @param path
	 *           the path
	 ...
	 * performed on URIs with an absolute path. Calling this method on a relative
	 * path URI will have no effect.
	 * 
	 * @throws URIException
	 *            no more higher path level to be normalized
	 * 
	...
	/**
	 * It can be gotten the URI character sequence. It's escaped. For the purpose
	 * of the protocol to be transported, it will be useful.
	 * 
