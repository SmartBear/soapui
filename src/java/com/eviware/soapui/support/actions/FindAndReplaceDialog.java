...
public class FindAndReplaceDialog extends AbstractAction
{
   ...
   private void buildDialog()
   {
      ...
      selectedLinesButton.setBorder( BorderFactory.createEmptyBorder( 3, 3, 3,
              3 ) );
   ...
   {
      int ix = forwardButton.isSelected() ? txt.indexOf( value, pos ) : txt
              .lastIndexOf( value, pos );

      if( wholeWordCheck.isSelected() )
      {
         while( ix != -1
                 && ( ix > 0 && Character.isLetterOrDigit( txt.charAt( ix - 1 ) ) )
                 || ( ix < txt.length() - value.length() - 1 && Character
                 .isLetterOrDigit( txt.charAt( ix + value.length() ) ) ) )
         {
      ...
      if( selectedLinesButton.isSelected()
              && ( ix < target.getSelectionStart() || ix > target
              .getSelectionEnd() ) )
         ix = -1;
   ...
   private class ReplaceAllAction extends AbstractAction
   {
      ...
      public void actionPerformed( ActionEvent e )
      {
         ...
         int ix = findNext( pos, txt, value );
         int firstIx = ix;
         int valueInNewValueIx = !caseCheck.isSelected() ? newValue
                 .toUpperCase().indexOf( value ) : newValue.indexOf( value );
